...
- Added support for [nested CDATA](https://en.wikipedia.org/wiki/CDATA#Nesting). `]]>` in CDATA is now automatically replaced with `]]]]><![CDATA[>`.
...
## [4.1.0] - 2015-11-11
- Added the `separateArrayItems` option to `create()` to control how arrays are handled when converting from objects.
